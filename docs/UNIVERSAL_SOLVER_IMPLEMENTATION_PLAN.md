...
Heuristic packing не доказывает глобальный optimum. Все ответы проходят наш validator.
...
Case не используется как безусловный ответ. При изменении заказа или прайса выполняется полный перерасчёт и повторная validation.
