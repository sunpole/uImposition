...
- Pareto foundation уже объединён в `main` через PR `#20`;
- следующая задача: компактный materially-different display set и структурированные объяснения компромиссов.
...
**Статус: в работе. Pareto foundation объединён в `main` через PR `#20`; новый release ещё не создан.**
...
- [ ] чистая модель компактного отображаемого набора поверх frontier;
- [ ] обязательное закрепление рекомендованного и уникальных крайних решений даже при малом display limit;
- [ ] устранение повторов, когда одно решение является лучшим по нескольким категориям;
- [ ] выбор дополнительных materially-different tradeoff-вариантов для оставшихся мест;
- [ ] человеческое объяснение преимущества и цены каждого варианта;
- [ ] точные дельты относительно рекомендованного или выбранного варианта;
- [ ] денежные дельты по бумаге, формам и итогу только при `pricing ready`;
- [ ] реальный набор нормализованных альтернатив, а не только unit fixtures;
